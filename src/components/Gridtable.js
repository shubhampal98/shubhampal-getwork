...
function descendingComparator(a, b, orderBy) {
  if(orderBy==='id' || orderBy==='employee_salary' || orderBy==='employee_age'){
    // console.log('a');
...
const headCells = [
  { id: 'id', numeric: true, disablePadding: true, label: 'S.No.' },
  { id: 'employee_name', numeric: false, disablePadding: false, label: 'Name' },
  { id: 'employee_age', numeric: true, disablePadding: false, label: 'Age' },
  { id: 'employee_salary', numeric: true, disablePadding: false, label: 'Salary' },
...
export default function EnhancedTable() {
  ...
  const onChangeHandler = (e) => {
    ...
    newRows = rows.filter((row) => {
      return row.employee_name.toLowerCase().includes(q.toLowerCase());
    });
    setRows(newRows);
  ...
  const handleSelected = () => {
    ...
    if(selected.length>0){
      alert(selected.map((s)=>{
        return rows[s-1].employee_name;
      }));
    }
  }
  ...
  return (
    <div className={classes.root}>
      ...
      {/* <FormControlLabel
        ...
        label="Dense padding"
      /> */}
      <Button variant="contained" color="primary" style={{marginRight:'1rem'}} onClick={handleSelected}>Select by College</Button>
      <Button variant="contained" color="primary" onClick={handleSelected}>Reject by College</Button>
    </div>
  );
}
